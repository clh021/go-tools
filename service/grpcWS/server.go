...
import (
	...
	proto "test/service/grpcWS/chatws" // 自动生成的 proto代码
)
...
func (s *Streamer) BidStream(stream proto.Chat_BidStreamServer) error {
	...
	for {
		...
		default:
			// 接收从客户端发来的消息
			recvFromClient, err := stream.Recv()
			if err == io.EOF {
			...
			default:
				// 缺省情况下， 返回 '服务端返回: ' + 输入信息
				log.Printf("[收到消息]: %s", recvFromClient.Input)
				if err := stream.Send(&proto.Response{Output: "服务端返回: " + recvFromClient.Input}); err != nil {
					return err
				}
			}
		}
